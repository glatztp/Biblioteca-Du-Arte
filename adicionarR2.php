...
<body>
    ...
    if (!empty($NomeLivro) && !empty($NomeAluno) && !empty($DataRetirada) && !empty($DataEntrega)) {
        $sql = "INSERT INTO reservas(NomeLivro, NomeAluno, DataRetirada, DataEntrega) VALUES (?, ?, ?, ?)";
        $stmt = mysqli_prepare($conexao, $sql);
        
        if ($stmt) {
            
            mysqli_stmt_bind_param($stmt, "ssss", $NomeLivro, $NomeAluno, $DataRetirada, $DataEntrega);
            
            if (mysqli_stmt_execute($stmt)) {
                $mensagemErro = 'Livro Reservado com sucesso';
                echo "<p class='mensagem-erro'>$mensagemErro</p>";
            } else {
                $mensagemErro = 'Erro ao Reservar Livro';
                echo "<p class='mensagem-erro'>$mensagemErro</p>";
            }
